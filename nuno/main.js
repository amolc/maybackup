...
// --------------- INTRO ANIMATION ---------------
let pngSeqframe = 0;
const totalFrames = 195; // Total frames from 00000 to 00180
const frameRate = 1000 / 24; // 30 frames per second
let pngSeqInterval;
const images = [];
...
function playAnimation() {
    ...
    pngSeqInterval = setInterval(function () {
        if (pngSeqframe < totalFrames) {
            aiIntroImg.src = `assets/Media/AIIntro/NunoAi_Intro_${pngSeqframe.toString().padStart(5, '0')}.webp`;
            pngSeqframe++;
        } else {
            introStart += 1;
            pngSeqframe = 0;
...
// --------------- IDLE ANIMATION ---------------
let idleFrame = 0;
const idleTotalFrames = 153; // Set total frames for idle animation
const idleFrameRate = 1000 / 24; // Adjust frame rate for idle animation
let idleInterval;
const idleImages = [];
...
function playIdleAnimation() {
    ...
    idleInterval = setInterval(function () {
        if (idleFrame < idleTotalFrames) {
            aiIdleImg.src = `assets/Media/AIIdle/NunoAi_Idle_${idleFrame.toString().padStart(5, '0')}.webp`;
            idleFrame++;
        } else {
...
function loadAllAnimations() {
    const animations = [
        { totalFrames: totalFrames, path: 'assets/Media/AIIntro/NunoAi_Intro_{frame}.webp', target: images, name: 'Intro Animation' },
        ...
        { totalFrames: idleTotalFrames, path: 'assets/Media/AIIdle/NunoAi_Idle_{frame}.webp', target: idleImages, name: 'Idle Animation' },
