  ...
  try {
    ...
    const displayNextWord = () => {
      if (wordIndex < words.length) {
        ...
        const urlRegex = /\b(https?:\/\/[a-zA-Z0-9\-._~:/?#[\]@!$&'()*+,;=%]*[a-zA-Z0-9\-._~:/?#[\]@!$&'*+,;=%])/g;
        $('.chatbot__chat p').each(function () {
          $(this).html($(this).text().replace(urlRegex, function (url) {
            // If the URL ends with a closing parenthesis, remove it
            if (url.endsWith(')')) {
    ...
    // Prepare the audio but do not play it automatically
...
function buildRequestBody(message) {
  // Replace all instances of "IVYBEARSÂ®" with "ivy bears"
  const formattedMessage = message.replace(/IVYBEARSÂ®/g, "ivy bears");
